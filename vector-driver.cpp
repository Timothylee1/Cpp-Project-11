...
void TestPush() {
  ...
  for (hlp2::vector::value_type i {0}; i < 5; i++) {
    a.push_back(i);
    Print(a);
  }
...
void TestSubscripts() {
  ...
  for (hlp2::vector::value_type i = 0; i < max_cnt; i++) {
    a.push_back(2 * i);
  }
  ...
  hlp2::vector b(10);
  for (hlp2::vector::size_type i{0}; i < b.size(); ++i) {
    b[i] = 3*i;
  }
  Print(b);
  ...
  for (hlp2::vector::size_type i{0}; i < c.size(); ++i) {
    c[i] = d[i]-1;
  }
  Print(c);  
}
